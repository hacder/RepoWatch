...
/*
		NSNib *genericNib = [[NSNib alloc] initWithNibNamed: @"generic" bundle: nil];
		NSArray *arr2;
		[genericNib instantiateNibWithOwner: self topLevelObjects: &arr2];
		
		NSView *mainView = [arr2 objectAtIndex: 1];
		[mainView retain];
		
		NSLog(@"Here is the generic stuff: %@\n", arr2);
		NSLog(@"Here is my twitter username: %@\n", _twitterUsername);
		
		[_twitterUsername setStringValue: [defaults objectForKey: @"twitterUsername"]];
		[_twitterPassword setStringValue: [defaults objectForKey: @"twitterPassword"]];
		
		NSTabViewItem *tvi = [[NSTabViewItem alloc] initWithIdentifier: self];
		[tvi setLabel: @"Generic"];
